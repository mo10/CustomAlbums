...
namespace MuseDashCustomAlbumMod
{
    public static class StageUIPatch
    {
        ...
        {
            // PnlStage.PreWarm
            var preWarm = AccessTools.Method(typeof(PnlStage), "PreWarm", new[] {typeof(int)});
            ...
            var getAlbumTagLocaliztionPostfix =
                AccessTools.Method(typeof(StageUIPatch), "GetAlbumTagLocaliztionPostfix");
            harmony.Patch(getAlbumTagLocaliztion, null, new HarmonyMethod(getAlbumTagLocaliztionPostfix));
        }

        // PnlStage.PreWarm
        public static void PreWarmPrefix(int slice, ref List<PnlStage.albumInfo> ___m_AllAlbumTagData,
            ref Transform ___albumFancyScrollViewContent, ref List<GameObject> ___m_AlbumFSVCells)
        {
            if (slice == 0)
            {
        ...
        // PnlStage.RangeStageList
        public static void RangeStageListPostfix(ref List<string> ___m_AllOtherAlbumUid,
            ref List<string> ___m_AllOtherAlbumName_Re, ref List<string> ___m_AllOtherAlbumUid_Re,
            ref List<string> ___m_AllOtherAlbumName, ref List<PnlStage.albumInfo> ___m_AllAlbumTagData)
        {
            // Rebind data
        ...
        public static void GetAlbumTagLocaliztionPostfix(string albumUid, ref string __result)
        {
            ...
            if (albumUid == AlbumTagUid) __result = CustomAlbum.Languages[activeOption];
        }
        ...
        private static void AddAlbumTagCell(ref Transform albumFancyScrollViewContent)
        {
            if (albumFancyScrollViewContent.Find(AlbumTagGameObjectName) == null)
            {
                ...
                // Clone gameobject
                var sourceGameObject = albumFancyScrollViewContent.GetChild(albumFancyScrollViewContent.childCount - 2)
                    .gameObject;
                var cloneGameObject = Object.Instantiate(sourceGameObject, sourceGameObject.transform.parent);
                // Initialization
                ...
                {
                    txtTagGameObject.GetComponent<Text>().text = CustomAlbum.Languages[activeOption];
                ...
                try
                {
                    if (iconGameObject != null)
                    {
                        ...
                        var newTex = new Texture2D(1, 1, TextureFormat.RGBA32, false);
                        
                        iconGameObject.name = "ImgCustom";
                        // Load default image from embedded resources.
                        newTex.LoadImage(Utils.ReadEmbeddedFile("Resources.AlbumIcon.png"));
                        newTex.filterMode = FilterMode.Point;
                        newSprite = Sprite.Create(newTex, new Rect(0, 0, newTex.width, newTex.height),
                            new Vector2(0, 0), 100);
